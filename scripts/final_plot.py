...
def create_plots(problem_tree):
    ...
    for file in os.listdir('.'):
        if file.endswith('.out') and file.startswith(problem_tree):
            pn = file.split('.uai')[0]
            Z_Value = findZ(pn)
    ...
    for abstraction in aL:
        print(abstraction)
        for file in os.listdir('.'):
            if file.endswith('.out') and file.startswith(problem_tree) and abstraction in file:
            	print (file)
    ...
    frame.to_csv(problem_tree + '.csv')
    df = pd.read_csv(problem_tree + '.csv')
